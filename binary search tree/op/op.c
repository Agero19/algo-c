...
static void op__check(const opitem *base, size_t nmemb) {
  for (const opitem *p = base; p < base + nmemb; ++p) {
    if (op__search(p->oper, p + 1, nmemb - (size_t) (p + 1 - base))
        != nullptr) {
      fprintf(stderr,
    ...
    for (size_t j = 0; j < OP_NUMBER_OF_PARAMETERS_MAX; ++j) {
      if ((p->param[j].lbl == nullptr) != (p->param[j].get == nullptr)) {
        fprintf(stderr,
...
#if OP_NUMBER_OF_PARAMETERS_MAX != 0
  size_t j = 0;
  while (j < OP_NUMBER_OF_PARAMETERS_MAX && p->param[j].get != nullptr) {
...
static void op__help(const opitem *base, size_t nmemb,
  ...
  for (const opitem *p = base; p < base + nmemb; ++p) {
...
#if OP_NUMBER_OF_PARAMETERS_MAX != 0
    size_t j = 0;
    while (j < OP_NUMBER_OF_PARAMETERS_MAX && p->param[j].lbl != nullptr) {
      PUTC(x, ' ');
    ...
    op__help_line(p->help == nullptr ? "" : p->help, x, tab2, ws.ws_col);
  }
  if (notes != nullptr) {
    const char **pp = notes;
    while (*pp != nullptr) {
...
opreturn op__item_exec_help([[maybe_unused]] void *unused) {
  return OP_RETURN_HELP;
}
